...
int baseSpeed = 110;     //base cruising speed
...
int speedChangeRate_accel = 8;    // speed changes over every loop at this rate to prevent jerking
int speedChangeRate_brake = 15;    // speed changes over every loop at this rate to prevent jerking
int joystickDeadzone = 50;    //to account for the error in the controller joystick
float turnSensitivity = 0.65 ;   //for better control. without this bot starts taking pturns for small x-input
...
void processGamepad(ControllerPtr ctl) {
    ...
    if (ccwpturn) {
        // ccw point turn
        int target = min(int(maxSpeed ), currentSpeed + speedChangeRate_accel);
        currentSpeed += (currentSpeed < target) ? speedChangeRate_accel : -speedChangeRate_accel;
        ...
        if (accel > 50) {
            ...
            currentSpeed = min(currentSpeed + speedChangeRate_accel, tgt);
        } else if (brake > 50) {
            int brk = map(brake, 0, 1023, 0, baseSpeed);
            int tgt = max(baseSpeed - brk, 0);
            currentSpeed = max(int((currentSpeed - speedChangeRate_brake)), tgt);
        } else {
            if (currentSpeed < baseSpeed) currentSpeed = min(currentSpeed + speedChangeRate_accel, baseSpeed);
            else if (currentSpeed > baseSpeed) currentSpeed = max(currentSpeed - speedChangeRate_accel, baseSpeed);
        }
